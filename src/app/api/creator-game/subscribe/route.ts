...
/**
 * Subscribe to Creator Game email course via ConvertKit
...
export async function POST(req: Request) {
  try {
    ...
    fields.subscription_source = "creator_game";
    ...
    if ("error" in result) {
      console.error("[creator-game] ConvertKit error:", result.error);
      return NextResponse.json(
        { error: result.error },
        { status: 500 }
      );
    ...
    console.log("[creator-game] Subscribed:", email, "Path:", path);
